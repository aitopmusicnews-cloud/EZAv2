...
/**
 * Keep Agnes generations at their authored speed. They are normalized and
 * hard-trimmed server-side to the exact timeline slot. Reusable library or
 * uploaded footage can still be adapted to a slot in preview, matching the
 * final renderer.
 */
function applyPlaybackRate(el: HTMLVideoElement, slotDur: number, source?: string): void {
  const agnesSource = source === "textToVideo" || source === "imageToVideo" || source === "keyframeToVideo";
  if (agnesSource) {
