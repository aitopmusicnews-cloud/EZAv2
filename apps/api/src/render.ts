...
export async function renderTimeline(req: RenderRequest): Promise<{ url: string }> {
  ...
  // Probe reusable library/upload footage so it can be adapted to the target
  // slot. Agnes-generated visuals are normalized to exact duration earlier and
  // are hard-trimmed here without changing playback speed.
  ...
  for (let i = 0; i < req.clips.length; i++) {
    ...
    const isAgnes =
      clip.source === "textToVideo" ||
      clip.source === "imageToVideo" ||
      clip.source === "keyframeToVideo";
    if (isAgnes) {
      baseChain =
        `${scalePad},trim=duration=${slotDur.toFixed(6)},` +
        `setpts=PTS-STARTPTS+${clip.start.toFixed(6)}/TB`;
    } else {
      // Reusable footage is allowed to adapt to a changed timeline slot.
