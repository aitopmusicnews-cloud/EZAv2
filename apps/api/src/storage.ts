...
export interface StorageBackend {
  ...
  /** Return a public HTTPS URL suitable for sending to Agnes. */
...
class LocalBackend implements StorageBackend {
  ...
  async providerUrl(rawUrl: string): Promise<string> {
    const parsed = new URL(rawUrl, config.PUBLIC_BASE_URL);
    if (parsed.protocol !== "https:") {
      throw new Error("Agnes image inputs require a public HTTPS URL. Set PUBLIC_BASE_URL to your HTTPS Render URL.");
    }
...
class S3Backend implements StorageBackend {
  ...
  async providerUrl(rawUrl: string): Promise<string> {
    const parsed = new URL(rawUrl, this.publicBase);
    if (parsed.protocol !== "https:") {
...
// --- Analysis cache ------------------------------------------------------
// Lives at analyses/<songId>.json (or .error.json). Goes
