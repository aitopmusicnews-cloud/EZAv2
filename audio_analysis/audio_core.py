"""Local audio-analysis logic used by the Fastify API.

This module has no cloud-provider or web-service dependency. The Node API invokes
``analyze_cli.py`` with an owned uploaded song and consumes the JSON result.
"""
...
def analyze_bytes(audio_bytes: bytes, sections_k: int = 0) -> dict[str, Any]:
    ...
    Returns the JSON shape consumed by the Fastify audio-analysis adapter.
    """
    import librosa
    import numpy as np
...
def _detect_sections(
    *,
    ...
    interior_bins = boundary_beats[1:]
    # ``librosa.util.sync`` creates one interval before the first beat and one
    # after the last beat, so synced-bin index N maps to beat_frames[N - 1].
    # Indexing beat_frames directly with the synced-bin index is both shifted by
    # one and can run past the final beat when agglomerative selects the tail bin.
    beat_indexes = interior_bins - 1
    raw_times = librosa.frames_to_time(beat_frames[beat_indexes], sr=sr).tolist()
