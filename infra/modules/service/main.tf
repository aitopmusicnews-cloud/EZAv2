...
#   aws ssm put-parameter --name /music-video-studio/hackathon/AGNES_API_KEY --value '...' --type SecureString
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        # (Agnes fetching public reference media from /storage) bypass CloudFront's basic auth.
      ...
      secrets = [
        {
          name      = "AGNES_API_KEY"
          valueFrom = "arn:aws:ssm:${var.region}:${data.aws_caller_identity.current.account_id}:parameter${local.ssm_prefix}/AGNES_API_KEY"
        }
      ]
      logConfiguration = {
        logDriver = "awslogs"
